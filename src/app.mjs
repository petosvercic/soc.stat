...
const verifyApiToken = (req, config) => {
  ...
  return typeof provided === "string" && provided === config.appApiToken;
};

const verifyPaywallToken = (req, config) => {
  if (!config.paywallToken) return true;
  const provided = req.headers["x-paywall-token"];
  return typeof provided === "string" && provided === config.paywallToken;
};

function isPublicPath(pathname, method) {
  if (pathname === "/" || pathname === "/login" || pathname === "/register") return true;
  if (pathname.startsWith("/share/") && method === "GET") return true;
  return false;
}

export async function appHandler(req, res, config) {
  ...
  if (pathname === "/health" && req.method === "GET") {
    return sendJson(res, 200, {
      ok: true,
      env: config.nodeEnv,
      paywallConfigured: Boolean(config.paywallProviderUrl && config.paywallPublicKey),
      vercelConfigured: Boolean(config.vercelToken),
      repoSyncConfigured: Boolean(config.repoSyncToken),
    });
  }

  if (pathname === "/paywall/config" && req.method === "GET") {
    if (!verifyPaywallToken(req, config)) return sendJson(res, 401, { error: "Invalid paywall token" });
    return sendJson(res, 200, {
      providerUrl: config.paywallProviderUrl,
      publicKey: config.paywallPublicKey,
    });
  }
